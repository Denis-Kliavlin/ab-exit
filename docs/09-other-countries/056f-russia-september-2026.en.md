# 56f. Russia, September 2026: the Duma elections and what follows

**Chapter:** 09 — Other countries
**File:** 09_056f · v1 · 22 September 2026
**Source:** post-election analyses of 21 September 2026 (Maxim Katz; Abbas Gallyamov; Meduza — Alexandra Prokopenko and Andrei Pertsev) and subsequent work with the architect
...
## 2. The Meduza discussion: what the protocol solves and what it does not
...
**And this is exactly why the single exception looks so convincing.** In Novgorod region Yabloko took over six per cent not because a tactic was found but because **the list stayed on the ballot** and there was no time to rewrite it (§1). What worked was not a voter's action but the presence of an alternative. That points to where the repertoire is empty: not in techniques of voting but in what there is to vote for.
...
The correction is accepted, and it follows from the same logic as §4. Removing Smart Voting was easy: it was a list of surnames presented as voting guidance, and a takedown demand for it fits in one line. Here what is proposed for removal is a **calculator** that divides published budget figures by the population. It contains no call to action, no surname, no organisation; the demand would read "remove the app that divides published numbers," and stores carry hundreds of such apps. Refusing on that ground is awkward, and complying is awkward too: the precedent is visible to everyone.

So a store should be treated not as the weak point but as a **tripwire**: if the app is removed after all, that is itself news, and the news is that a **calculator** was removed. For the cause that is no worse than installation — the same mechanism §4 describes for bans in general.
...
So for fifteen years people were shown numbers that **divide into nothing**. Outrage per rouble in them is enormous; roubles per person are not. This is the arithmetic version of the same empty slot as §3: the opposition's material was moral ("they steal, and it is vile") rather than countable ("here is your share"). And it struck a target not worth striking: taking everything from the deputies leaves a person thirteen roubles a month.
...
The architect's verdict on the previous section: **"the comparison with luxury is not much good."** Correct, and the reason is in its own weak point: every official car for a year is three per cent of the dividend. It is the very genre of outrage that divides into nothing (§3).
...
### What follows for the plan in §4
...
## 7. Collect the signatures even if the deadline is burned: what is right here and where the danger lies
...
One detail to dispose of at once. The law does not reward excess: at least forty per cent of signatures are checked, and five per cent defective means refusal regardless of the total (§6). Doubling also doubles defects in absolute terms. As a **legal** move doubling is pointless; as a **measuring** one it makes complete sense, and that is how it should be presented.
...
**Second, following directly from the first:** the defence here is the one named in §4 — **having no owner**. A collection without an owner has nobody to designate undesirable. This is another independent confirmation that the main line of defence is the structure rather than the wording.
...
The practical consequence for the §4 plan: the anonymous count is something that can be built **today**, without initiative groups, notaries or deadlines, using the same app that computes the share (§5). The legal collection is something to undertake only when ready to answer for signatories' safety, and that question is settled before the first signature rather than after.

**Weak point.** "If the whole opposition unites" is an assumption rather than a plan; §3 shows it did not happen once in a full cycle, and the question's non-political character makes unity **possible** without securing it. An anonymous count has no legal force at all and is easily dismissed as inflated — its credibility rests solely on the openness of the counting scheme, which requires publishing all tokens as in 048i.2. And neither instrument answers what happens to the organisers: measurement protects participants, not those conducting it. 🟡

## 8. "The thirteenth salary" and the word "freebie": the best unit and a dangerous word
...
**It is a settlement rather than a handout.** A thirteenth salary is not begged for — it is **received at the end of a period**. That is exactly the frame §5 demanded in place of the word "free": not a gift but something due. And unlike "your share" it does not require first agreeing with an argument about the subsoil — it is intelligible before any argument.

**It is periodic and tied to the end of a cycle** — as is a payment arriving the day before an election (004.4.8).

**And it is wholly non-political**, so it passes the foreman test (§7) without a word of explanation.

### But the arithmetic must be stated honestly

A thirteenth salary for a worker on forty thousand is forty thousand. The formula at K = 1 % yields about eleven thousand — **roughly a quarter of a thirteenth salary**. A full one requires K of the order of six or seven per cent (§5).

So "a thirteenth salary" may be said only together with the slider: **under the present cautious formula it is a quarter, and what it becomes is decided by a referendum**. Promising a full one without naming K is precisely what §5 warns against.
...
The architect is right in the observation: it will be received exactly so, and that applies **to the foreman too** — he is a recipient as well. Which incidentally closes the foreman test (§7) harder still: the foreman not only cannot explain why not, he is owed the same money, so he has no personal interest in dissuading anyone.
...
The architect's question: what effect will there be on Z-bloggers and pro-government opinion leaders generally. Examined by the same method as the foreman scene (§7): not "what will they think" but **which moves are available to them**.

### Two moves they do not have

**They cannot say "the wealth is not yours."** The thesis that Russia is the world's richest country in subsoil is theirs rather than ours: it has been a matter of state pride for decades and an anchor of the sovereignty argument (§7). Abandoning it for the sake of one dispute is impossible — too much else rests on it.
...
And "they will go and check" is the key to the whole section: **they check with the calculator** (§5). Whoever came for the hype opens the count, gets a number about himself, and can no longer not know it. The calculator here is not agitation but **the trap's trigger**: it turns a borrowed slogan into personal arithmetic, and personal arithmetic does not fold back into a slogan.
...
Indirect confirmation is self-taxation (§6): thousands of local referendums in which people vote to **pay themselves**. The money flows the other way and it still happens — because they decide. **What works is not the direction of payment but whose decision it is.**

### And hence the basis for the rules about words

The bans in §5 on "free" and §8 on "freebie" were derived tactically: those words invite ready-made objections. They now have a deeper basis: **both words return the recipient to the supplicant's position**, abolishing precisely what the construction exists for. A handout called a share remains a handout; a share called a freebie becomes one.
...
The architect's correction to §9: "you are confusing them — what the authorities say, what the opposition on YouTube says, and what a permitted opinion leader says in Telegram channels."
...
| Whom it reaches | everyone, with zero credibility on this topic | the already convinced (§3) | **the mass audience** |
...
**And hence something unexpected: the blockade itself produces the condition §4 calls the main defence.** An idea everyone knows and nobody attributes **has no owner** — there is nobody to designate undesirable because there is nobody to point at. What is intended as isolating the source works as detaching the idea from the source.

The practical consequence for the §4 plan changes point 5. It previously read "spread through those who cannot be struck off." More precisely now: **the idea must be built so that it can be retold without a citation**. That is a requirement on form — a short rule, a countable number, a calculator that works by itself (§5) — rather than on a list of allies. What gets retold is what needs no authority in order to be checked.
...
So the mass medium §10 called the bridge is **being dismantled as we write**, and the audience is moving into a messenger controlled by the state. The claim that permitted Telegram channels are the only bridge was true when written and is already false.
...
And hence a practical consequence for §5 worth recording outright: the calculator must be built to **work and spread inside the state messenger** — as a link, an image and a number forwarded without explanation and without a source, rather than as an app from a store or a clip on a blocked platform.
...
## 11. Does the opposition in exile have better ideas: an honest survey
...
|---|---|---|---|
| Corruption investigations | no: outrage does not divide per person — the palace is 684 ₽ once (§5) | high, it is being done | a full cycle with millions of viewers produced zero districts (§3) |
...
| Sanctions and external pressure | no: the addressee is Western governments, not a voter | high, it is being done | no effect on the internal picture; and its touch makes a topic toxic (§10) |
| Work with emigrants | yes, but for those who already left | high | foreign precincts routed the party of power (§1) and nothing changed inside |
| The anti-war agenda | no: it requires courage and there is no valve (§2) | high | no protest voting occurred |
...
By the first measure the answer is unambiguous: **not one of the offerings even attempts** to give an apathetic person a personal reason to turn up. Everything listed is procedural ("how to vote"), moral ("against the war," "they steal") or external (sanctions). Nobody asks "why bother at all," and this was established by survey rather than by argument (§3).
...
**That assessment is withdrawn at the architect's instance, and rightly — see §12.** In brief: an elite split fails not on dissatisfaction but on coordination, and coordination requires a public pretext. Mass support is not an alternative to the split but its **input**.
...
Investigations, sanctions and work with emigrants are being done **today**. The protocol needs adoption, adoption needs a referendum, and the procedure is closed by clocks and subgroups (§6). By this measure it is among the worst in the table.

What is realisable within it is **narrow**: the calculator (§5), a trial local referendum (§6), a ready text of the question, an anonymous count (§7). That is not a programme but four cheap actions.
...
| A federal referendum | adoption of the protocol | closed (§6) |
...
If it is needed more by the poorest, the opposition's agreement is **a convenience of distribution rather than a condition of existence**. And §10 showed that a labelled source's touch additionally **costs**: it makes the topic toxic for every other channel.

Four facts established in this chapter add up to one:

1. §5 — the sum means more the poorer the recipient;
2. §7 — the technique passes the foreman test, requiring no political position;
3. §10 — an opposition channel does not distribute but infects, and the mass medium is private channels;
4. §9 — the idea spreads through small actors who need audience, not through a champion.
...
That is exactly the ownerlessness requirement of §4 and §10, now arrived at from a third direction.
...
The architect's objection to §11: "an elite split is more likely with 90 % popular support. And elites need a pretext for consolidating with the people and among themselves. A pretext that is explicit and cannot be openly condemned."
...
| **Measurability** | one cannot coordinate on a feeling, one can on a number | yes: the counter and the anonymous count produce a figure (§7) |
| **Consolidation with the people rather than away from them** | a split on a liberal platform leads away from the 90 %, not towards them | yes: "for the people" rather than against the authorities (§10) |
| **Personal gain for the defector** | "democracy" costs him rather than paying him | yes: he is a recipient too, as is the foreman (§8) |
...
A defector saying "Russia is rich and a share is due to everyone" **cannot be condemned without condemning one's own propaganda**: the riches of the subsoil are an official thesis and a matter of state pride for decades (§7). A traitor is shot; a man repeating the television with a question at the end is shot **with explanations**, and there are none to give.
...
And the addressee of §4's four actions changes. They looked like work with the population. It is now clear that **the calculator and the anonymous count produce exactly the public landmark an apparatus needs in order to coordinate**: a figure that can be cited without anyone conspiring with anyone.
...
| Do not publish | every recipient supplies **his own** guess, the guesses spread through the same private channels (§10), and refuting them requires publication — that is, the first move |
...
So §9's test ("who sets the size?") takes a precise form: **who controls the quantities the size is computed from?** And from here it is visible why the formula is tied to median earnings from an independent source rather than to the volume of rent (§5). That was recorded as protection against manipulation. In fact it is what makes it **unforgeable in a competition of formulas**: a rival whose inputs are his own loses to anyone whose inputs are external — and loses publicly, because the comparison is computed on the same calculator.
...
"I would have bought…" is the form that reference point takes: not abstract arithmetic but a specific foregone purchase. It makes the gap personal and vivid — exactly the property §5 wanted when it required measuring in salaries rather than roubles.

**Hence an addition to the calculator's specification (§5):** show not only the sum and the gap but **what the gap buys**. Not "you were underpaid thirty thousand" but "that is a refrigerator" or "that is six months of utility bills." The unit should be a thing rather than a number.
...
The correction is accepted, and the error was a substitution of terms. **A computation is not a promise.** §5 requires outright that the formula be open, the figures changeable and the source of every line given: the calculator says "here is the arithmetic, check it," not "we will pay you this much." Whoever pays is the one who promises.
...
**And "unkillable in the information field" needs refining, because §10 says otherwise.** Opposition channels are **weak as distributors** — most people do not read them. But they are **indestructible as a source**: they publish from abroad, cannot be closed, and what is published stays available.

For an argument about a formula that is sufficient and nothing more is needed. **Keeping a formula in circulation requires not mass reach but the impossibility of being silenced.** After that others repeat it, and repeat it in the very transmission medium §10 describes as the principal one.
...
The third field has been rewritten in the form of [23b.1](../05-empirical-base/023b-evidence-rules.en.md): what transfers, which way the gap shifts it, how much remains. In the first edition it was binary ("does not carry"), which read as a cut-off on the transfer, whereas a difference in conditions is a discount.

| The chapter's assumption | Nearest confirmed analogue | What transfers and how much remains |
|---|---|---|
| Money moves the apathetic towards participation | **James et al. (2025)**: +10 % in the size of Alaska's dividend (~$190) → **+1.4 pp of turnout**, with the effect **among the young and less well-off**; Loeffler (2022) — the dividend raises turnout (014) | **Transfers:** the relationship, its sign, and the target group — the very one §5 concerns. **The gap runs both ways:** Alaska is a democracy where turnout is not suppressed, so there is less headroom → the analogue **understates**; but the payment there is not tied to voting whereas ours is → it **overstates**. **Remains: largely, with the sign of the net shift unknown** |
| People turn up for money | Alaska 2024: **666,213 applications, 624,489 payments** against a population of ~735,000 — about 85 % of residents receive it, children included (014) | **Transfers:** the existence of the phenomenon and its scale — almost everyone turns up for money. **The gap:** the number of eligible non-applicants is unpublished, so no take-up rate can be derived and the 85 % is itself a lower bound. **Remains: largely, as a lower bound** |
| A payment without a formula is unstable and turns into an argument about the rule | **Alaska 2016–2018** (014.14.3b): a discretionary halving → a lawsuit, collapsing approval, the governor leaving the race, a rival winning on the full formula | **Transfers:** the whole chain — a cut, a lawsuit, political consequences. **The gap:** the causation is documented as press interpretation rather than measured, and the immediate occasion of the withdrawal was the lieutenant governor's resignation. **Remains: largely, with a reservation on causation** |
| The formula must be protected more strongly than anything else (004.4.7b) | **Wielechowski v. State (2017)**: a statutory formula **does not bind** legislature or governor | **Transfers** in full: this is a judicial ruling about a property of the construction, not about a country. **No substantive gap** — the lesson applies in any jurisdiction with annual appropriation. **Remains: almost entirely** |
| Envy works for distribution rather than against it | Foster (1965) + **Chinoy et al., AER (2026)**: zero-sum thinking predicts both support for redistribution and harshness towards outsiders (056d.5b) | **Transfers:** the structure of the relation and both correlations at once — they match both of our objections. **The gap:** no measurement for Russia, so what is unknown is the magnitude, not the direction. **Remains: direction confidently, magnitude not at all** |
| Common knowledge changes an apparatus's behaviour | East Germany, May 1989 (056d.7b); Chwe's coordination theory | **Transfers:** the existence of the phenomenon — accumulated common knowledge changes an apparatus's behaviour. **The gap is large:** a single case, no money question in it, and §1 supplies a direct observation against immediate action. **Remains: the existence of the phenomenon only, and with contrary evidence beside it** |
...
Second: survey data from Russia do not belong here, a rule introduced in the course of the work (§13). Where it has been applied retrospectively some earlier claims were withdrawn — but no full revision of the chapter against that rule was carried out, and residues of survey material may remain.

Third, and material to every practical section: **two things remain without an analogue** — a payment tied to the act of voting, and an elite split caused by a dividend demand (§13). Everything else rests on a measurement or an observable case; these two do not.

Fourth: the chapter describes Russia, where the protocol cannot be adopted (056d.8), so its practical part — §4, §5, §6, §7 — is work for a transition window and for measurement rather than for polling day. It cannot be read as a programme of action yielding results within any foreseeable term.
